...
"""
Create the CORE 2021 ONLY DONE
"""
...
2020 DONE
"""
base_directory = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\GEARS TASKS\Market Prices Data\EnexGroup&Historical\2020_EL-DAM_PrelimResults\PrelimResults"
...
result.set_index('DELIVERY_MTU', inplace=True)    
path = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\Επιφάνεια εργασίας\DAM price prediction\DAM\CORE 2020.xlsx"
path = path.replace('\\','/')
result.to_excel(path)     
...
#2022 DONE
base_directory = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\GEARS TASKS\Market Prices Data\EnexGroup&Historical\2022_EL-DAM-CRIDAs_Results\2022_EL-XBID_Results\2022_EL-XBID_Results"
...
while current_date <= end_date:
    ...
    date_str = current_date.strftime(date_format)
    file_name = f"{date_str}_EL-XBID_Results_EN_v01.xlsx"  # Format the file name
    file_path = os.path.join(base_directory, file_name)
    file_path = re.sub(r'\\', '/', file_path)
...
path = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\Επιφάνεια εργασίας\DAM price prediction\XBID\CORE 2022.xlsx"
path = path.replace('\\','/')
result.to_excel(path,index=False) 
...
base_directory = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\GEARS TASKS\Market Prices Data\EnexGroup&Historical\2024 CRIDA\IDA 1"
...
while current_date <= end_date:
    ...
    date_str = current_date.strftime(date_format)
    file_name = f"{date_str}_EL-IDA1.xlsx"  # Format the file name
    file_path = os.path.join(base_directory, file_name)
    file_path = re.sub(r'\\', '/', file_path)
        
    if os.path.exists(file_path):  # Check if the file exists
            
        df1 = pd.read_excel(file_path, sheet_name='EL-IDA1_Results')
            #df1['WAP'] = df1['VWAP (€/MWh)'] * df1['TOTAL_TRADES']
        trades = df1.groupby(['DELIVERY_MTU', 'MCP'])['TOTAL_TRADES'].sum().reset_index()
        df = pd.concat([df, trades], axis=0)
...
path = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\Επιφάνεια εργασίας\DAM price prediction\IDA 1\CORE 2024.xlsx"
path = path.replace('\\','/')
result.to_excel(path,index=False) 
...
base_directory = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\GEARS TASKS\Market Prices Data\EnexGroup&Historical\2024 CRIDA\IDA 2"
start_date = datetime(2024, 6, 14)
end_date = datetime(2024, 7, 10)
date_format = "%Y%m%d"
current_date = start_date
...
while current_date <= end_date:
    ...
    date_str = current_date.strftime(date_format)
    file_name = f"{date_str}_EL-IDA2.xlsx"  # Format the file name
    file_path = os.path.join(base_directory, file_name)
    file_path = re.sub(r'\\', '/', file_path)
        
    if os.path.exists(file_path):  # Check if the file exists
            
        df1 = pd.read_excel(file_path, sheet_name='EL-IDA2_Results')
            #df1['WAP'] = df1['VWAP (€/MWh)'] * df1['TOTAL_TRADES']
        trades = df1.groupby(['DELIVERY_MTU', 'MCP'])['TOTAL_TRADES'].sum().reset_index()
        df = pd.concat([df, trades], axis=0)
...
path = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\Επιφάνεια εργασίας\DAM price prediction\IDA 2\CORE 2024.xlsx"
path = path.replace('\\','/')
result.to_excel(path,index=False) 
...
base_directory = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\GEARS TASKS\Market Prices Data\EnexGroup&Historical\2024 CRIDA\IDA 3"
start_date = datetime(2024, 6, 14)
end_date = datetime(2024, 7, 10)
date_format = "%Y%m%d"
current_date = start_date
...
while current_date <= end_date:
    ...
    date_str = current_date.strftime(date_format)
    file_name = f"{date_str}_EL-IDA3.xlsx"  # Format the file name
    file_path = os.path.join(base_directory, file_name)
    file_path = re.sub(r'\\', '/', file_path)
        
    if os.path.exists(file_path):  # Check if the file exists
            
        df1 = pd.read_excel(file_path, sheet_name='EL-IDA3_Results')
            #df1['WAP'] = df1['VWAP (€/MWh)'] * df1['TOTAL_TRADES']
        trades = df1.groupby(['DELIVERY_MTU', 'MCP'])['TOTAL_TRADES'].sum().reset_index()
        df = pd.concat([df, trades], axis=0)
...
path = r"C:\Users\Eleni\OneDrive - Hellenic Association for Energy Economics (1)\Επιφάνεια εργασίας\DAM price prediction\IDA 3\CORE 2024.xlsx"
path = path.replace('\\','/')
result.to_excel(path,index=False) 
#########################################################################################################################################################
